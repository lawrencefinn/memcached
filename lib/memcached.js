...
(function (nMemcached) {
  ...
  memcached.command = function memcachedCommand(queryCompiler, server) {
    ...
    this.connect(server, function allocateMemcachedConnection(error, S) {
      ...
      //Why not use a sep connection?  You have a damn pool
      S.writable = false;
      var originalCallback = query.callback;
      query.callback = function(err, reply) {
        S.writable = true;
        return originalCallback(err, reply);
      }
